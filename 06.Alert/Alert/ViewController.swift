...
//  Alert
//
//  Created by silver j on 2022/11/08.
//
...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampOn(_ sender: UIButton) {
        if (isLampOn==true) {
            let lampOnAlert = UIAlertController(title: "경고", message: "현재 켜져있는 상태입니다", preferredStyle: UIAlertController.Style.alert)
    ...
    @IBAction func btnLampOff(_ sender: UIButton) {
        if isLampOn {
            let lampOffAlert = UIAlertController(title: "전등 끄기", message: "전등을 끄시겠습니까?", preferredStyle: UIAlertController.Style.alert)
